...
public class Remove implements CommandInterface {
	...
	public void handle(CommandContext ctx) {
		...
		if (ctx.getArgs().size() == 1) {
			try {
				...
				AudioTrackInfo audioTrackInfo = trackScheduler.removeTrack(songNum);
				EmbedBuilder builder = new EmbedBuilder()
					.setTitle(audioTrackInfo.title, audioTrackInfo.uri)
					.setColor(RandomColor.getRandomColor())
					.setAuthor(
						"❌ Removed song from position " + songNum + " in queue",
						audioTrackInfo.uri,
	...
	public String getDescription() {
		return "Removes a selected song from the queue";
	}
